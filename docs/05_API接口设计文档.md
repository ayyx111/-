...
> 文档版本:V1.2
> 编写日期:2026-08-16
> 更新日期:2026-08-20(新增修改邮箱接口;用户名校验调整为3-20位;封禁中间件实时检查)
> 基础路径:`/api/v1`
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| newEmail | string | 是 | 新邮箱(需验证码验证) |
| code | string | 是 | 发送到新邮箱的验证码 |

**说明:** 邮箱修改需先通过 `POST /auth/send-code` 向新邮箱发送验证码,验证通过后才能修改。邮箱唯一性校验:已被其他账号使用的邮箱无法绑定。被封禁账号的邮箱不能注册新账号(数据库唯一约束)。
